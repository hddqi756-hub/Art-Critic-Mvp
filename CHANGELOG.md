...
- 全局修改增强为三段式流程：全图审计 → 修改强度选择 → 结果对比。
- `visualPromptTranslator` 翻译节点增强，新增英文美术术语映射表。
...
- 修复知识卡、作业卡只识别图片 URL、不识别 base64 返回导致“服务器处理失败”的问题。
...
- 知识卡配图失败时保留文字卡和本地 PNG 降级产物，并落盘错误原因方便排查。
- 结果页新增“本次成果”总览区，集中展示全局修改、局部示范、知识卡和作业卡完成数量。
- 问题侧栏增加“改图 / 知识 / 作业”状态徽标，便于逐个问题检查生成进度。
- 作业卡图片切换为统一 `ImagePreview`，支持预览、缩放、拖拽和下载。
- 局部改图标签页新增状态面板，显示图像 API、Mask、处理中、失败原因和重试入口。
- 后端局部改图失败时会写回对应问题的 `taskMemory.issueStates`，前端可稳定恢复失败原因。
- 知识卡新增状态面板，显示文字卡、教学配图、处理中、本地降级等状态。
- 作业卡新增状态面板，显示文字练习、图像 API、图片完整性和失败/降级原因。
- 后端知识卡和作业卡会写回 `taskMemory.issueStates`，刷新或切换页面后仍可恢复卡片状态。
- 新增 `GET /api/teacher/memory-summary`，汇总 AI 老师长期记忆、当前任务记忆和历史批改分区。
- AI 老师页面侧栏升级为“本次任务 / 核心问题 / 已学知识 / 老师备注”和“最近目标 / 高频问题 / 最近批改”分区。
- AI 老师历史分区会联动最近批改记录、长期弱项和知识点计数，方便继续追问时保留学习上下文。
- 全局修改会生成并保存 `taskMemory.globalAudit`，记录全图审计摘要、审计维度、已列问题和修改强度。
